class PlanningsController < ApplicationController
  ...
  def create  
    ...
    if Planning.find_by(user_id: current_user.id).level
      @planning.level = Planning.find_by(user_id: current_user.id).level  # 一度でもプランを作成したことがある時
    else
      @planning.level = 10  # 一度もプランを作成したことがなければデフォルト値
    end
